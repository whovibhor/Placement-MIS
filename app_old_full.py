...
# ── CDM Bulk Student Import API ─────────────────────────────────────────────
@app.route("/api/cdm/drive/<int:drive_id>/students/bulk", methods=["POST"])
def cdm_bulk_link_students(drive_id):
    """Bulk-link students to a drive from an uploaded Excel file.
    The Excel must have a 'Reg Number' column. Match is case-insensitive."""
    ...
    try:
        ...
        # Find 'Reg Number' column (case-insensitive)
        ...
        conn = get_connection()
        cursor = conn.cursor(dictionary=True)
        # Fetch all students and build case-insensitive lookup
